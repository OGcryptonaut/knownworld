...
Discipline rules I held to throughout: verify verdicts (match / possible-mismatch / unverified) are computed in code by comparing evidence to the database — never asked of the model. Closeness is computed in code from message volume + recency; the model's schema has no closeness field to smuggle one through (a hostile fake model in tests tried — it reached storage 0 of 62 times). Malformed model output is rejected with logged reasons. Thin evidence produces "unverified," never a guess. Every call logs its cost: total model spend for the entire build, full-corpus runs included, is **~$0.19**. 152 agent tests + 34 web tests, all green.
