...
// The enriched network in three views (table / map / graph) over ONE merged
// dataset: distilled people + enrichment cards, joined by tg_id. Approvals
// here mirror /verify exactly — only explicit user action writes the DB, and
// unflagged mismatch approvals are refused server-side (409).
...
import { VerdictBadge, type DbRow } from '@/components/database/shared';

const AGENTS_URL = process.env.NEXT_PUBLIC_AGENTS_URL ?? '/agents';
const VIEW_KEY = 'kw-db-view';

type View = 'table' | 'map' | 'graph';
type LoadState = 'loading' | 'ready' | 'offline';

const VIEWS: View[] = ['table', 'map', 'graph'];

function isView(v: string | null): v is View {
  return v === 'table' || v === 'map' || v === 'graph';
}
...
export default function DatabasePage() {
  ...
  const selectedRow = useMemo(
    () => (selected === null ? null : (rows.find((r) => r.person.tg_id === selected) ?? null)),
    [rows, selected],
  );
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-slate-500">
          distilled rows + verified evidence — three views of one dataset
        </span>
      </div>
      ...
      {state === 'offline' ? (
        ...
        <>
                ...
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  {pending.map((r) => (
                    <button
                      key={r.person.tg_id}
                      type="button"
                      onClick={() => setSelected(r.person.tg_id)}
          ...
          {view === 'table' && <DatabaseTable rows={rows} onSelect={setSelected} />}
          {view === 'map' && <DatabaseMap rows={rows} onSelect={setSelected} />}
          {view === 'graph' && <DatabaseGraph rows={rows} onSelect={setSelected} />}
        </>
      )}
